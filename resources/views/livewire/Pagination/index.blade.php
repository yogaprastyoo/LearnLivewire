    ...
    <div class="max-w-2xl mx-auto min-h-screen flex justify-center items-center">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="shadow h-full rounded-lg w-full p-4 space-y-4 bg-white">
                @include('livewire.Pagination.includes.create-user')
            </div>

            {{-- UserList --}}
            <div class="shadow relative h-[24.5rem] md:h-full rounded-lg w-full p-4 space-y-4 bg-white">
                @include('livewire.Pagination.includes.list-user')
            </div>
        </div>
    </div>
</div>
